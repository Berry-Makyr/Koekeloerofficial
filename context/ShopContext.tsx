...
export function ShopProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      ...
      // Categories
      const savedCategories = localStorage.getItem('koekeloer_custom_categories_v3');
      if (savedCategories) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          ...
          localStorage.setItem('koekeloer_custom_categories_v3', JSON.stringify(defaultCategories));
        }
      } else {
        setCategoriesList(defaultCategories);
        localStorage.setItem('koekeloer_custom_categories_v3', JSON.stringify(defaultCategories));
      ...
      // Site Content
      const savedContent = localStorage.getItem('koekeloer_site_content_v4');
      if (savedContent) {
  ...
  useEffect(() => {
    if (!isHydrated) return;
    try {
      localStorage.setItem('koekeloer_custom_categories_v3', JSON.stringify(categoriesList));
    } catch (e) {
  ...
  useEffect(() => {
    if (!isHydrated) return;
    try {
      localStorage.setItem('koekeloer_site_content_v4', JSON.stringify(siteContentState));
    } catch (e) {
