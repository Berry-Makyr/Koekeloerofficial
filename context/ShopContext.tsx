...
export function ShopProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      // Products
      const savedProducts = localStorage.getItem('koekeloer_custom_products_v5');
      if (savedProducts) {
        ...
        if (Array.isArray(parsed)) {
          ...
          localStorage.setItem('koekeloer_custom_products_v5', JSON.stringify(defaultProducts));
        }
      } else {
        setProductsList(defaultProducts);
        localStorage.setItem('koekeloer_custom_products_v5', JSON.stringify(defaultProducts));
      ...
      // Site Content
      const savedContent = localStorage.getItem('koekeloer_site_content_v3');
      if (savedContent) {
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem('koekeloer_custom_products_v5', JSON.stringify(productsList));
    } catch (e) {
  ...
  useEffect(() => {
    if (!isHydrated) return;
    try {
      localStorage.setItem('koekeloer_site_content_v3', JSON.stringify(siteContentState));
    } catch (e) {
