...
export function ShopProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      // Products
      const savedProducts = localStorage.getItem('koekeloer_custom_products_v8');
      if (savedProducts) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          ...
          localStorage.setItem('koekeloer_custom_products_v8', JSON.stringify(defaultProducts));
        }
      } else {
        setProductsList(defaultProducts);
        localStorage.setItem('koekeloer_custom_products_v8', JSON.stringify(defaultProducts));
      ...
      // Categories
      const savedCategories = localStorage.getItem('koekeloer_custom_categories_v4');
      if (savedCategories) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          ...
          localStorage.setItem('koekeloer_custom_categories_v4', JSON.stringify(defaultCategories));
        }
      } else {
        setCategoriesList(defaultCategories);
        localStorage.setItem('koekeloer_custom_categories_v4', JSON.stringify(defaultCategories));
      ...
      // Site Content
      const savedContent = localStorage.getItem('koekeloer_site_content_v5');
      if (savedContent) {
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem('koekeloer_custom_products_v8', JSON.stringify(productsList));
    } catch (e) {
  ...
  useEffect(() => {
    if (!isHydrated) return;
    try {
      localStorage.setItem('koekeloer_custom_categories_v4', JSON.stringify(categoriesList));
    } catch (e) {
  ...
  useEffect(() => {
    if (!isHydrated) return;
    try {
      localStorage.setItem('koekeloer_site_content_v5', JSON.stringify(siteContentState));
    } catch (e) {
